...
class Board:
    ...
    def __init__(self, players):
        ...
        # self.show_window() #uncomment for graphics
        self.update()

    def get_grid(self):
        grid = [list([] for x in range(0, self.grid_size)) for y in range(0, self.grid_size)]

        colors = list(csv.reader(open("data/color.csv")))
        track = list(csv.reader(open("data/track.csv")))

        for y in range(0, self.grid_size):
            for x in range(0, self.grid_size):
                grid[x][y] = Cell(x, y, colors[y][x], track[y][x])
                # grid[x][y].draw(self.screen) #uncomment for graphics
    ...
    def show_pieces(self):
        for player in self.players:
            for piece in player.pieces:
                if piece.is_out():
                    self.get_cell(player.color, piece.position).draw_piece(self.screen, player.color)
    ...
    def show_base(self, player):
        # Get the base position for each color
        if player.color == "RED":
        ...
        pygame.draw.rect(self.screen, color, pygame.Rect(base_position, (base_size, base_size)))
        ...
        for x in range(0, player.pieces_in()):
            ...
            pygame.draw.circle(self.screen, color, piece_position, size)
    ...
    def update(self):
        self.draw_board()
        # pygame.display.flip() #uncomment for graphics
        # pygame.image.save(self.screen, "img/Img"+str(self.cnt)+".png") #uncomment for video
        # self.cnt += 1 #uncomment for video

    def draw_board(self):
        # Fill screen with black
        # self.screen.fill(self.line_color) #uncomment for graphics

        self.grid = self.get_grid()

        # self.show_bases() #uncomment for graphics
        # self.show_extra_graphics() #uncomment for graphics

        # self.show_pieces() #uncomment for graphics

        # self.show_whos_turn() #uncomment for graphics
        # self.show_roll() #uncomment for graphics